...
const servicePresetSelect = document.getElementById('service-preset');
...
const SERVICE_PRESET_URLS = {
  aade_mydata_home: 'https://www.aade.gr/mydata',
  aade_bookkeeper_dashboard: 'https://www1.aade.gr/saadeapps2/bookkeeper-web/bookkeeper/#!/',
  aade_bookkeeper_aggregate: 'https://www1.aade.gr/saadeapps2/bookkeeper-web/bookkeeper/#!/bookAggregate',
};
...
btnStart.addEventListener('click', () => {
  const selectedPreset = servicePresetSelect.value;
  const presetUrl = selectedPreset ? SERVICE_PRESET_URLS[selectedPreset] : '';
  const url = (startUrlInput.value.trim() || presetUrl).trim();
...
servicePresetSelect.addEventListener('change', () => {
  ...
  startUrlInput.value = SERVICE_PRESET_URLS[selectedPreset] || '';
});
...
runScriptSelect.addEventListener('change', async () => {
  ...
  try {
    ...
    // try to extract parameters from the run() function's destructuring
    // this works for scripts generated by actionsToScript
      ...
      m[1].split(',').forEach(part => {
        ...
        if (pieces[0]) {
          try {
            ...
            let val = eval(pieces[1]);
            if (val === undefined) val = '';
            paramsObj[pieces[0]] = val;
          } catch {
            paramsObj[pieces[0]] = '';
          }
        }
    ...
    // if no destructured params found, fall back to parsing JSDoc @param lines
    if (Object.keys(paramsObj).length === 0) {
      // fallback 1: parse JSDoc @param comments
      const docRe = /@param\\s+\{[^}]+\}\\s+params\.([a-zA-Z0-9_]+)/g;
      let docMatch;
      while ((docMatch = docRe.exec(src)) !== null) {
        const name = docMatch[1];
        paramsObj[name] = '';
      }
    }

    // if still no params, try inferring from literal page.fill/selectOption values
    if (Object.keys(paramsObj).length === 0) {
      const inferRe = /page\.(?:fill|selectOption)\(\s*(['\"])([^'\"]+)\1\s*,\s*(['\"])([\s\S]*?)\3\s*\)/g;
      let infMatch;
      let idx = 1;
      while ((infMatch = inferRe.exec(src)) !== null) {
        const selector = infMatch[2];
        const defaultVal = infMatch[4] || '';
        let name = sanitizeParamName(selector);
        if (!name) name = `param${idx++}`;
        // ensure unique
        while (paramsObj[name]) {
          name = `param${idx++}`;
        }
        paramsObj[name] = defaultVal;
      }
    }

    if (Object.keys(paramsObj).length) {
      runParamsTA.value = JSON.stringify(paramsObj, null, 2);
      loopParamsTA.value = `[
